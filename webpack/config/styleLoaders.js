...
// Plain style loader
const styleLoader = require.resolve('vue-style-loader');
...
const sassLoader = {
    ...
    options: {
        sourceMap: true,
        //Sharing Global scss Variables for VUE,
        // So we can use your shared variables & mixins across all SASS styles without manually importing them in each vue file.
        data: '@import "app/scss/common.scss";'
    }
};
...
const extractSassRules = (paths, extractTextPlugin, isModule) => {
    ...
    const generateLoader = (isModule) => {
        let extractLoader = extractTextPlugin.extract(
            Object.assign({
                    fallback: styleLoader,
                    use: [
                        isModule? cssLoader(isModule) : cssLoader(),
                        postCssLoader(),
                        resolveUrlLoader,
                        sassLoader
                    ]},
        ...
        // css reload with HMR
        if (!isProduction) {
            extractLoader = ['css-hot-loader'].concat(extractLoader);
        }
